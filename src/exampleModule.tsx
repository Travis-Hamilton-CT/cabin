...
const AirlineClassDropdown: React.FC = () => {
    ...
    const [showDropdownClasses, setDrpoDownClasses] = useState(false);
    ...
    const status = useScript('https://cdn.tailwindcss.com', {
    ...
    if (status === 'ready') {
        return (
            <HelmetProvider>
                ...
                <div className="relative w-full flex flex-col items-center justify-start text-right text-xl text-gray-1900 font-plus-jakarta-sans">
                    <div className="flex flex-row items-center justify-end gap-[12px] z-[0] hover:cursor-pointer"
                        onClick={() => {
                    ...
                    >
                        <Icon />
                        <h5 className="m-[0] relative text-[16px] leading-[16px] font-normal font-inherit inline-block">
                            {cabinClass}
                        </h5>
                    ...
                    {showDropdownClasses && (
                        <div className="mt-[10px] rounded-[0px_0px_15px_15px] bg-white-200 [border:1px_solid_#e6e8ec] box-border w-[328.04px] flex flex-row p-[24px] items-start justify-start pr-[0px] z-[1] text-left text-2xl">
                            <div className="flex-1 flex flex-col items-start justify-start gap-[28px]">
                                {data.map((item, idx) => (
                                    <div
                                        key={idx}
                                        onClick={() => handleCheck(item)}
                                        className="self-stretch flex flex-row items-center justify-between hover:cursor-pointer"
                                    >
                                        <div className="flex flex-row items-center justify-start gap-[10px] whitespace-nowrap">
                                            {/* <div className="relative w-[30px] h-[30px] shrink-0"> */}
                                                <item.icon />
                                            {/* </div> */}
                                            <p className="m-[0] relative leading-[16px] inline-block">
                                                {item.name}
                                            </p>
                                        </div>
                                        <input
                                            className="h-[1.15rem] mr-[35px] w-[1.15rem] my-1 accent-violet-200   cursor-pointer"
                                            key={Math.random()}
                                            type="checkbox"
                                            onChange={() => handleCheck(item)}
                                            checked={item.checkBool}
                                        />
                                ...
                                <button
                                    ...
                                    <label className="relative text-base leading-[16px] font-bold font-plus-jakarta-sans text-[inherit] text-center inline-block cursor-pointer">
                                        Apply
                                    </label>
                                </button>
                            </div>
